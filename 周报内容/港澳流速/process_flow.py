...
DATA_FILE = SAMPLE_DIR / "海外港澳商务_各渠道主辅投数据 (2).xlsx"
OUTPUT_DIR = Path(__file__).parent / "output"

# 本月书展供应商（每月更新）
BOOK_FAIR_SUPPLIERS = ["26年6月", "26年6月課外活動展"]
...
def load_data_table() -> dict:
    ...
    # 独立站HK
    dlz_rows = supplier_totals[supplier_totals["渠道组"] == "独立站HK"]
    for _, row in dlz_rows.iterrows():
        name = row["供应商"]
        if name == "总计":
            result["dlz_total"] = {
                "例子数": row["例子数"], "约课数": row["约课数"], "滚动消耗": row["滚动消耗"]
    ...
    # 全渠道总计
    all_total_rows = supplier_totals[supplier_totals["渠道组"] == "总计"]
    if len(all_total_rows) > 0:
        row = all_total_rows.iloc[0]
        result["all_total"] = {
            "例子数": row["例子数"], "约课数": row["约课数"], "滚动消耗": row["滚动消耗"]
        }
...
def process_flow_sheet(data: dict, book_fair_suppliers: list):
    ...
    for row_idx in range(2, ws.max_row + 1):
        ...
        if b_val in ("", "KOL历史", "累计例子", "平均成本", "日均例子", "5月目标预计",
                     "KOL", "展会", "代理人", "商超", "线下HK历史达成数据", "全渠道汇总"):
            break  # 到了下方的统计区域，停止

        data_rows.append((row_idx, b_val))

        # 收集 KOL 供应商名（排除汇总行、商超社群行、独立站行）
        if b_val not in ("其他汇总", "KOL-汇总", "代理商场", "书展", "代理人汇总",
                         "合计", "商超&社群合计", "出席礼品测试", "edm", "澳門展会"):
    ...
    for row_idx, supplier_name in data_rows:
        if supplier_name in ("KOL-汇总", "合计", "总计"):
    ...
    kol_total_lessons = 0
    kol_total_examples = 0
    shangchao_total_consumption = 0
    shangchao_total_lessons = 0
    shangchao_total_examples = 0
    dlz_total_consumption = 0
    dlz_total_lessons = 0
    dlz_total_examples = 0

    seen_summary_count = 0  # 用于区分两个"合计"行（行12=商超&社群合计，行13=全渠道汇总）
    ...
    for row_idx, supplier_name in data_rows:
        ...
        elif supplier_name == "出席礼品测试":
            # 独立站HK 中名称含"出席"的供应商汇总（如：618出席送禮）
            for s_name, s_data in data["dlz"].items():
                if "出席" in s_name:
                    examples += s_data["例子数"] or 0
                    lessons += s_data["约课数"] or 0
                    cost_consumption += s_data["滚动消耗"] or 0

        elif supplier_name == "edm":
            # 独立站HK 中名称含"edm"的供应商汇总（暂无供应商上线）
            for s_name, s_data in data["dlz"].items():
                if "edm" in s_name.lower():
                    examples += s_data["例子数"] or 0
                    lessons += s_data["约课数"] or 0
                    cost_consumption += s_data["滚动消耗"] or 0
        ...
        # 累计到对应的汇总
        if supplier_name in ("代理商场", "书展", "代理人汇总", "澳門展会"):
            shangchao_total_consumption += cost_consumption
            shangchao_total_lessons += lessons
            shangchao_total_examples += examples
        elif supplier_name in ("出席礼品测试", "edm"):
            dlz_total_consumption += cost_consumption
            dlz_total_lessons += lessons
            dlz_total_examples += examples
        else:
            kol_total_consumption += cost_consumption
            kol_total_lessons += lessons
            kol_total_examples += examples
    ...
    for row_idx, supplier_name in data_rows:
        if supplier_name == "KOL-汇总":
            ws.cell(row_idx, COL_AP_ACTUAL + 1).value = kol_total_examples
            ws.cell(row_idx, COL_AS_LESSON_ACTUAL + 1).value = kol_total_lessons
            if kol_total_lessons > 0:
                ws.cell(row_idx, COL_AU_COST + 1).value = kol_total_consumption / kol_total_lessons
            print(f"  KOL-汇总          → 例子={kol_total_examples}, 约课={kol_total_lessons}, "
                  f"成本={round(kol_total_consumption/kol_total_lessons, 2) if kol_total_lessons > 0 else '-'}")

        elif supplier_name == "商超&社群合计":
            ws.cell(row_idx, COL_AP_ACTUAL + 1).value = shangchao_total_examples
            ws.cell(row_idx, COL_AS_LESSON_ACTUAL + 1).value = shangchao_total_lessons
            if shangchao_total_lessons > 0:
                ws.cell(row_idx, COL_AU_COST + 1).value = shangchao_total_consumption / shangchao_total_lessons
            print(f"  商超&社群合计       → 例子={shangchao_total_examples}, 约课={shangchao_total_lessons}, "
                  f"成本={round(shangchao_total_consumption/shangchao_total_lessons, 2) if shangchao_total_lessons > 0 else '-'}")

        elif supplier_name == "合计":
            seen_summary_count += 1
            if seen_summary_count == 1:
                # 第一个"合计"= 商超&社群合计（不含独立站）
                ws.cell(row_idx, COL_AP_ACTUAL + 1).value = shangchao_total_examples
                ws.cell(row_idx, COL_AS_LESSON_ACTUAL + 1).value = shangchao_total_lessons
                if shangchao_total_lessons > 0:
                    ws.cell(row_idx, COL_AU_COST + 1).value = shangchao_total_consumption / shangchao_total_lessons
                print(f"  合计（商超&社群）    → 例子={shangchao_total_examples}, 约课={shangchao_total_lessons}")
            else:
                # 第二个"合计"= 全渠道汇总（KOL + 独立站 + 商超&社群）
                all_examples = kol_total_examples + dlz_total_examples + shangchao_total_examples
                all_lessons = kol_total_lessons + dlz_total_lessons + shangchao_total_lessons
                all_consumption = kol_total_consumption + dlz_total_consumption + shangchao_total_consumption
                ws.cell(row_idx, COL_AP_ACTUAL + 1).value = all_examples
                ws.cell(row_idx, COL_AS_LESSON_ACTUAL + 1).value = all_lessons
                if all_lessons > 0:
                    ws.cell(row_idx, COL_AU_COST + 1).value = all_consumption / all_lessons
                print(f"  合计（全渠道汇总）   → 例子={all_examples}, 约课={all_lessons}, "
                      f"成本={round(all_consumption/all_lessons, 2) if all_lessons > 0 else '-'}")
