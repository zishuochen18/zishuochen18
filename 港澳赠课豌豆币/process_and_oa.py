...
OA_LOGIN_URL = "https://login.61info.cn/"


def get_last_week_range():
    """计算上周一到上周日，返回 (周一日期, 周日日期, '5.18-5.24' 格式字符串)"""
    today = datetime.now()
...
OA_PORTAL_URL = "https://dingding.61info.cn/sys/portal/page.jsp"
...
def step2_submit_oa(excel_path: Path, total_coins: int, total_users: int):
    ...
    with sync_playwright() as p:
        ...
        # 用 JS 在 iframe 内直接点击该链接（避开 Playwright text 匹配问题）
        ...
        # ─── 填写表单字段 ───
        # Radio 字段用 name + value 直接 check
        # Text 字段用 name 直接 fill
        ...
        # 积分成本归属部门（manifest 地址选择器）
        try:
            # 点击容器激活 manifest 搜索输入
            container = fp.locator('xformflag[property*="fd_3a43a4a8d4e396"] div.inputselectsgl').first
            container.click()
            time.sleep(1)
            # 在容器内找到 manifest 创建的可编辑输入框
            # manifest 会在 div.input 内创建一个 input
        ...
        # 活动时间（填写上周日期范围）
        _, _, range_str = get_last_week_range()
        activity_time = f"2026.{range_str}"
