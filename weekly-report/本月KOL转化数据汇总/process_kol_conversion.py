...
1. 从数据底表提取钟嘉欣（图片+视频1+视频2）和周家蔚的数据
...
- CURRENT_MONTH_SERIAL：当前月份的 Excel 日期序列号
  例：5月 = 46143（2026-05-01）

【注意事项】
- 写入"值"而不是公式（避免数据混乱）
- 按列标题精确匹配，重复列名取第一次出现的列
- 钟嘉欣三个供应商（图片/视频1/视频2）分别填入【钟嘉欣--求和】sheet
"""
import sys
...
DATA_FILE = Path(r"c:\Users\chenzishuo\Desktop\新建文件夹 (2)\周报内容\港澳流速\sample\海外港澳商务_各渠道主辅投数据.xlsx")
KOL_SUMMARY_FILE = SAMPLE_DIR / "海外港澳商务_各渠道主辅投数据-KOL汇总.xlsx"
CONVERSION_FILE = SAMPLE_DIR / "本月KOL转化链路数据汇总.xlsx"
OUTPUT_DIR = BASE_DIR / "output"

# 当前月份（5月对应的 Excel 日期序列号）
CURRENT_MONTH_SERIAL = 46143
...
def fill_kol_summary_file(data):
    ...
    # --- 1. 填充【钟嘉欣--求和】sheet (图片/视频1/视频2 三行) ---
    ws_sum = wb["钟嘉欣--求和"]
    ...
    fill_row_by_columns(ws_sum, 5, data.get("钟嘉欣视频2", {}), header_row=2)
    print(f"  [钟嘉欣--求和] 已填充图片/视频1/视频2供应商行")
    ...
    print(f"  [钟嘉欣] 已填充5月数据到行{target_row}（粘贴值）")
    ...
    print(f"  [周家蔚] 已填充5月数据到行{target_row}")
...
def fill_conversion_file(data, zjx_aggregate):
    ...
    for row_idx in range(3, ws_month.max_row + 1):
        ...
        if cell_val and "5月钟嘉欣" in str(cell_val):
            zjx_may_row = row_idx
            break

    if zjx_may_row:
        fill_row_by_columns(ws_month, zjx_may_row, zjx_aggregate, header_row=2)
        print(f"  [本月汇总数据] 钟嘉欣5月数据已填充到行{zjx_may_row}")

    # 找到5月周家蔚行
    zjw_may_row = None
    for row_idx in range(3, ws_month.max_row + 1):
        cell_val = ws_month.cell(row_idx, 1).value
        if cell_val and "5月周家蔚" in str(cell_val):
            zjw_may_row = row_idx
            break

    if zjw_may_row:
        fill_row_by_columns(ws_month, zjw_may_row, data.get("周家蔚", {}), header_row=2)
        print(f"  [本月汇总数据] 周家蔚5月数据已填充到行{zjw_may_row}")

    # 填充5月KOLHK汇总行
    kolhk_may_row = None
    for row_idx in range(3, ws_month.max_row + 1):
        cell_val = ws_month.cell(row_idx, 1).value
        if cell_val and "5月-KOLHK汇总" in str(cell_val):
    ...
    if kolhk_may_row:
        ...
        print(f"  [本月汇总数据] KOLHK 5月汇总已填充到行{kolhk_may_row}")
    ...
    for row_idx in range(3, ws_detail.max_row + 1):
        ...
        if a_str == "钟嘉欣图片":
            ...
            v2_data = data.get("钟嘉欣视频2", {})
            if v2_data:
                fill_row_by_columns(ws_detail, row_idx, v2_data, header_row=2)
                print(f"  [钟嘉欣图片&视频数据] 钟嘉欣视频2已填充到行{row_idx}")
